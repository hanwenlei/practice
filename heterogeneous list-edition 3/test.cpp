...
int UserFunction1(SL_List* l_list, SL_Node* ThisNode)
{
...
int UserFunction2(SL_List* l_list, SL_Node* ThisNode)
{
	#ifdef _DEBUG
	free(ThisNode);
	#else
	switch(ThisNode->flag)
	{
		case 0:
			free(ThisNode);
			break;
...
int test_ClearList(FunctionPoint l_UserFuncPoint2)
{
	SL_List* l_list = (SL_List*)malloc(sizeof(SL_List));
	SL_Node* test1 = (SL_Node*)malloc(sizeof(SL_Node));
	SL_Node* test2 = (SL_Node*)malloc(sizeof(SL_Node));
	...
	InitHeadNode(l_list);
	ClearList(l_list, UserFunction2);
	FAIL_TEST(l_list->HeadNode, NULL);
	FAIL_TEST(l_list->NodeCount, 0);
	
	InsertNode(l_list->HeadNode, test4, l_list);
	ClearList(l_list, UserFunction2);
	FAIL_TEST(l_list->HeadNode, NULL);
	FAIL_TEST(l_list->NodeCount, 0);
